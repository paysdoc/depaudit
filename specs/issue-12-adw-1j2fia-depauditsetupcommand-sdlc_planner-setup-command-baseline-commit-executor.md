...
2. **Trigger branch resolution.** `main` is preferred when it exists on the remote (`gh api repos/:owner/:repo/branches/main`); when it does not, the repo's default branch (`gh api repos/:owner/:repo --jq .default_branch`) is used. The resolved name is the **Trigger branch** in the ubiquitous-language sense (`UBIQUITOUS_LANGUAGE.md:32`) and is the value substituted into the gate workflow template's `on.pull_request.branches` field — pinning the gate to fire on PRs targeting the resolved branch (per the issue body: the workflow is "pinned to the resolved branch"). The packaged template itself stays branch-agnostic (no `branches:` restriction); the mutation is applied only when scaffolding into the target repo. A header comment documenting the resolved branch and the `depaudit setup` invocation timestamp is also prepended to the scaffolded file.
...
4. **Scaffolded artifacts.**
   - `.github/workflows/depaudit-gate.yml` — copied from `templates/depaudit-gate.yml` with `on.pull_request.branches` set to `[<triggerBranch>]` so the scaffolded gate is pinned to the resolved branch. A header comment is also prepended documenting the trigger branch and the timestamp at which `depaudit setup` resolved it. The Slack notification block (the `SLACK_WEBHOOK_URL` env entry on the `Post or update PR comment` step) is already in the packaged template from issue #11.
...
- **New `src/commands/depauditSetupCommand.ts`** — composition root. Pure orchestration; every I/O boundary is delegated to a deep module that accepts an injectable `execFile` / `fetch` / `now` for testability. Order of operations matches the PRD `:153-164` step list:
  ...
  5. Scaffold `.github/workflows/depaudit-gate.yml` from the packaged template via the template-copy helper. The helper sets `on.pull_request.branches` to `[<triggerBranch>]` and prepends a header comment line (`# generated by depaudit setup at <ISO>; trigger branch: <branch>`).
  <!-- ADW-WARNING: The issue does not specify whether `depaudit setup` should abort or skip when scaffolded files already exist. The plan favors idempotent skip (steps 6 & 7 below); the @adw-12 scenarios in features/setup.feature L222-241 expect a non-zero abort with stderr containing "already exists". Implementer: pick one — the chosen behavior must be applied uniformly across .depaudit.yml, osv-scanner.toml, and the workflow file, and the templateInstaller (Task 3) and integration tests (Task 10) updated to match. -->
...
- **New `src/modules/commitOrPrExecutor.ts`** — deep module exporting:
  ...
  export interface CommitOrPrExecutorOptions {
    ...
    commitMessage?: string;   // defaults to "depaudit setup: bootstrap"
    prTitle?: string;
    prBody?: string;
    execFile?: ExecFileFn;
  }
  ...
  Behaviour:
  1. `git -C <repoRoot> branch --show-current` → `currentBranch`.
...
- `features/support/mockGitBinary.ts` — companion to `mockGhBinary.ts`; emits a `git` shim binary into a temp dir whose subcommands (`branch --show-current`, `add`, `commit`, `checkout -b`, `push`, `ls-remote`, `rev-parse HEAD`) are configurable per scenario via a state file.
...
- Read the template; parse with `yaml`'s `parseDocument`; set the `on.pull_request.branches` field to a `YAMLSeq` containing the single string `<triggerBranch>`; re-serialise with `doc.toString()`. Prepend a two-line header comment:
  ```
...
- <!-- ADW-WARNING: The issue does not specify pre-existing-file behavior. Plan favors idempotent skip + conflict flag; @adw-12 scenarios expect a non-zero abort with stderr "already exists". Implementer must choose one and apply uniformly. --> Idempotency: if the destination exists already, read it; if its body (after stripping any prior `# generated by depaudit setup` header lines) matches the new body byte-for-byte, do not write (return `{ destPath, written: false }`). If bodies differ, do not overwrite — return `{ destPath, written: false, conflict: true }` and let the caller print a warning.
...
- `src/modules/__tests__/templateInstaller.test.ts`. Cover:
  ...
  - Scaffolded YAML's `on.pull_request.branches` parses to a single-item list containing `<triggerBranch>`.
...
- Behaviour:
  1. `git -C <repoRoot> branch --show-current` → `currentBranch`. (Per `features/commit_or_pr_executor.feature` L152-156, the executor must use `branch --show-current` rather than `rev-parse --abbrev-ref HEAD`.)
  2. `git -C <repoRoot> add <pathsToCommit>` (one call with all paths).
  3. Branch decision:
     - `currentBranch !== triggerBranch`:
       - `git -C <repoRoot> commit -m <commitMessage>` (default `commitMessage`: `"depaudit setup: bootstrap"` — must contain the literal string `"depaudit setup"` per `features/commit_or_pr_executor.feature` L32-37).
       - `git -C <repoRoot> rev-parse HEAD` → `commitSha`.
       - Return `{ kind: "commit", branch: currentBranch, commitSha }`.
     - `currentBranch === triggerBranch`:
...
- `src/commands/__tests__/depauditSetupCommand.test.ts`. Cover:
  - Feature-branch path: mock `git branch --show-current` → `feature/x`; mock all subsequent `git` / `gh` calls; mock `runScanCommand` injection to return a known `ScanResult` with two new findings (one OSV, one Socket). Assert: scaffolded files exist, baseline writes one `[[IgnoredVulns]]` block + one `supplyChainAccepts` entry with the canonical reason and a `+90d` expiry, executor takes the commit path.
  - Trigger-branch path: same setup but mock `git branch --show-current` → `main`; assert executor takes the PR path and `gh pr create` is invoked.
...
- Supported subcommand stubs (configurable via `MockGitState`):
  - `branch --show-current` → emits `currentBranch` from state.
  ...
  - `checkout -b <branch>` → records the new branch in the log; subsequent `branch --show-current` returns it.
...
- **Severity threshold higher than the only finding's severity** — baseline writes zero entries; stdout summary shows `baseline: osv: 0 / socket: 0`. Setup still commits the scaffolded files.
  <!-- ADW-WARNING: features/setup_baseline.feature L99-104 asserts a fixture can be "pre-configured with a depaudit setup override setting policy.severityThreshold to 'high'", but features/setup.feature L222-230 says setup aborts when .depaudit.yml already exists. The issue does not describe an override mechanism that bypasses the abort. Implementer must reconcile: (a) accept the abort and rely on a non-yml override mechanism (env var / CLI flag), (b) loosen the abort to permit a pre-existing .depaudit.yml whose body is a small "override-only" partial document, or (c) flag the setup_baseline.feature scenario as test-only fixture pre-configuration that bypasses the abort (test-only path). Document the chosen mechanism in code + scenarios. -->
...
- **Trigger branch is pinned in the scaffolded workflow's `on.pull_request.branches`.** Per the issue body ("workflow file references the resolved branch" and bootstrap step "Scaffold `.github/workflows/depaudit-gate.yml` pinned to the resolved branch"), `depaudit setup` mutates the scaffolded workflow's `on.pull_request.branches` to a single-item list containing the resolved branch. A header comment also records the resolution timestamp for human readability. Important: the **packaged template** at `templates/depaudit-gate.yml` stays branch-agnostic — `features/depaudit_gate_workflow.feature:135` correctly asserts the *packaged* template has no hard-coded branch; the mutation happens only when the template is scaffolded into the target repo, so that scenario continues to pass.
...
- **PR title and body for the trigger-branch path.** Default `prTitle: "depaudit setup: bootstrap"` (must contain the literal string `"depaudit setup"` per `features/commit_or_pr_executor.feature` L60-65); default `prBody`:
  ```
